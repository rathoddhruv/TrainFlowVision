...
if __name__ == '__main__':
    ...
    update_yaml_path(YOLO_DATASET_YAML_ABS, "data/temp/yolo_dataset")
    ...
    dataset_root = TRAINING_IMAGES_DIR.parent
    dataset_yaml = YOLO_MERGED_YAML_ABS

    initial_images = TEMP_DIR / "yolo_dataset" / "images" / "train"
    initial_labels = TEMP_DIR / "yolo_dataset" / "labels" / "train"
    ...
    if initial_images.exists() and any(initial_images.glob("*")) and valid_initial_labels:
        ...
        _sync_yaml(Path(YOLO_DATASET_YAML_ABS), Path("data/temp/yolo_dataset"))
        ...
        USED_PATH = Path("data/temp/yolo_dataset_used")
        if USED_PATH.exists():
            shutil.rmtree(USED_PATH)
        shutil.move("data/temp/yolo_dataset", USED_PATH)
    ...
    if args.clean:
        ...
        subprocess.run([sys.executable, "ml/utils/cleanup_dataset_folders.py"])
    else:
    ...
    if subprocess.run([sys.executable, "ml/scripts/boost_merge_labels.py"]).returncode != 0:
    ...
    subprocess.run([sys.executable, "ml/utils/fix_non_normalized_labels.py"], check=True)
    ...
    if MODEL_PATH.exists():
        ...
        print(f"Skipping evaluation because model not found at {MODEL_PATH}")
